...
@section('contenido')
    <div class="md:flex md:items-center">
        <div class="md:w-6/12 px-10 bg-purple-400">
            <p>imagen</p>
        </div>
        <div class="md:w-5/12 p-10 bg-white rounded-lg shadow-xl">
            <form class="w-full max-w-sm" action="{{ route('registar') }}" method="post" novalidate>
                ...
                <div class="mb-5">
                    ...
                    <input value="{{ old('titulo') }}" type="text" placeholder="Titulo de la Publicación..." name="titulo" id="titulo"
                ...
                <div class="mb-5">
                    ...
                    <textarea name="descripcion" placeholder="Descripcion de la Publicación..."  id="descripcion" class="mt-1 capitalize border p-2.5 w-full rounded-lg shadow-sm focus:outline-none focus:bg-white focus:border-purple-500 @error('descripcion') border-red-600 @enderror" >{{ old('descripcion') }}</textarea>
                        @error('descripcion')
                        <p class="bg-red-500 text-white my-2 rounded-sm text-sm p-2 text-center normal-case">{{ $message }}
                        </p>
                    @enderror
